...
//				i++;
	...
	public ArrayList<String> searchByName() {
		...
		String userResponse = "";
		do {
			System.out.println("Enter partial or full name of a pokemon");
			if (counter > 0) {
	...
	public ArrayList<String> searchByMultiple() {
		...
		if (resultListFiltered.isEmpty()) {
			System.out.println("No specific pokemon found with the provided information."
	...
	public String selectPokemon() {
		...
		while (!userResponse.equals("m")) {
			...
			userResponse = scanner.nextLine();
			if (userResponse.equals("1")) {
				System.out.println("Enter full name of a pokemon name: ");
				userResponse = scanner.nextLine();
				for (int i = 0; i < pokeList.size(); i++) {
	...
	public void login() throws Exception {
		...
		if (userList.get(userId).getIsAdmin()) {
			System.out.println("Welcome " + ANSI_CYAN + userList.get(userId).getFirstName() + "!" + ANSI_RESET + ANSI_YELLOW);
	...
	public void modifyAbility(int pokeNum) {
		...
		while (true) {
			System.out.println("\nEnter a ability number from the list to modify: ");
			userResponse = scanner.nextLine();
			switch (userResponse) {
			case "1":
				fieldName = "Ability1";
	...
	public void updateModification(int pokeNum, String table, String field, Object value) {
		...
		try {
			...
			if (rows > 0) {
				System.out.println(field + " field updated for " + pokeList.get(pokeNum).getName());
			}
		} catch (Exception e) {
			System.out.println(e);
		}
	 ...
	 * Name: Kevin Sanchez
	 * Date:11/27/2022
	 * Purpose: Add a new Pokemon and its attributes to the data structure as well as database
	 */
void addNewPokemon() throws Exception { 
		...
		/*
		* If the user agrees to create a Pokemon ask the user to enter the name of Pokemon
		* The name variable is updated with the userInput 
		* 
		*/
			if(userInput.equals("y")) {
				String name = "Coming soon", ability1 = "Coming soon", ability2 = "Coming soon", hiddenAbility = "Coming soon", generation = "Coming soon", color = "Coming soon", evolve = "Coming soon", region = "Coming soon", gender = "Coming soon";
				
				int ID = pokeList.size() + 1, type1 = 0, type2 = 0, total = 0, speed = 0, defense = 0, spDefense = 0, spAttack = 0, attack = 0, exp = 0, hp = 0;
				
				double mass = 0.0;

				    /*
				     * While agreeing to create a new Pokemon prompt user to create name and ID will be implemented upon completion of name
				     */
					...
					while(userInput.equals("y")) {
						...
						System.out.println("\nWould you like to add attributes to " + name + "? Please specify by entering y for yes or n for no. (Enter q to exit menu.) ");
				...
				if(choice.equals("y")) { 
					 ...
					 */
						...
						System.out.println("Would you like to save " + name + "'s ID placement in the database under the ID number " + ID + " and update the attributes later?");
						String userInput1 = scan.nextLine();
						
						
						if(userInput1.equals("y")) {
				
							pokeList.add(new Pokemon(gender, exp, region, evolve, color, mass, generation, hp, attack, spAttack, defense, spDefense, speed,
									total, ability1, ability2, hiddenAbility, type1, type2, name, ID));
							...
							String query2 = "INSERT INTO PokeGenInfo(ID, Gender, Region, EvolveAt, Color, Mass, Generation, Exp) VALUES('"+ID+"', '"+gender+"', '"+region+"', '"+evolve+"', '"+color+"', '"+mass+"', '"+generation+"', '"+exp+"')";
							statement.executeUpdate(query2);
							String query3 = "INSERT INTO PokeStats(ID, HP, Atk, Def, SpA, SpD, Spe, total) VALUES('"+ID+"', '"+hp+"', '"+attack+"', '"+defense+"' , '"+spAttack+"', '"+spDefense+"', '"+speed+"','"+total+"')";
							statement.executeUpdate(query3);
							String query4 = "INSERT INTO PokeAbility(ID, Ability1, Ability2, HiddenAbility) VALUES('"+ID+"', '"+ability1+"', '"+ability2+"', '"+hiddenAbility+"')";
							 ...
							 */
								...
								/*
								 * If user chooses not to save NAME and ID, do not upload to database and exit menu
								 */
...
//Type 2
			...
			/*
			 * Asking user if they'd like to create a new Pokemon, 
			 * if so it will ask for the new Pokemon name and it will be given the next ID space
			 */
	...
	public void comparePokemon() {
		String userResponse;
		int firstPokemon = 144;
		int secondPokemon = 150;
		int score1;
		int score2;
		int total1 = 0;
		int total2 = 0;
		...
		while (true) {
			...
			if (total1 > total2) {
				...
				System.out.println("\nBased on total stats scores are leveled!");
			}

			while (true) {
				System.out.println("Compare different pokemon? (y/n)");
				userResponse = scanner.nextLine();
				if (userResponse.equals("y")) {
					break;
				} else if (userResponse.equals("n")) {
				...
				if (selectedPokemon.equals(pokeList.get(i).getName())) {
					for (int j = i; j < pokeList.size(); j++) {
						if (pokeList.get(j).getEvolveAt().equals("N/A")) {
							...
							System.out.println(pokeList.get(j).getName() + " evolves into "
	...
	public void pokeRegion() {
		...
		String regionN;
		String userResponse = "";
		System.out.println("View all pokemons of a Region");
...
// sub method for pokeRegion
	public String selectRegion() {
		...
		while (!userResponse.equals("m")) {
			...
			if (userResponse.equals("1")) {
				...
				System.out.println("Kanto, Johto, Hoenn, Sinnoh, Unova, Kalos \n");
				userResponse = scanner.nextLine();
				for (int i = 0; i < pokeList.size(); i++) {
	 ...
	 * Name: Kevin Sanchez 
	 * Date: 11/22/2022 
	 * Purpose: Compare Pokemon type to its superior and inferior types
	 */
	 ...
	 * Name: Kevin Sanchez
	 * Date:12/1/2022
	 * Purpose: To view the Pokemon's set of moves it can obtain through leveling
	 */
